<!--
  ...
  // TODO :
  - Complete adding champion backgrounds
  - Beautify user info page
  - Summoner spell description tab
  - Color the live game tile of the player you're searching
  - Show other peoples' ranks
  - FIX SEARCH ON Summoner.php
  - USER MANUAL ->
  ...
  - INDEX.php ->
    > RE-WORK
         ...
         88            88
         88            88
,adPPYb,88 ,adPPYYba, 88,dPPYba,
a8"    `Y88 ""     `Y8 88P'    "8a
8b       88 ,adPPPPP88 88       d8
"8a,   ,d88 88,    ,88 88b,   ,a8"
`"8bbdP"Y8 `"8bbdP"Y8 8Y"Ybbd8"'

STOP LOOKING INTO THE CODE! >:(
...
if ($_SESSION['summ'] != null && $_SESSION['summName'] != null && $_SESSION['APIKey']) {
  $summoner = $_SESSION['summ'];
  $summonerName = $_SESSION['summName'];
  $apiKey = $_SESSION['APIKey'];
} else {
  $summ = "none";
  echo "Failed to retrieve necessary information :(";
  ...
  $url = 'https://euw.api.pvp.net/championmastery/location/EUW1/player/' . $_SESSION['summ']->id . '/topchampions?count=1&api_key=' . $apiKey;
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_AUTOREFERER, TRUE);
  curl_setopt($ch, CURLOPT_HEADER, 0);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_URL, $url);
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION, TRUE);
  $userTopChampionsJSON = curl_exec($ch);
  curl_close($ch);
  $userTopChampions = json_decode($userTopChampionsJSON);
  ...
  $url = 'https://euw.api.pvp.net/api/lol/euw/v1.3/stats/by-summoner/' . $_SESSION['summ']->id . '/summary?api_key=' . $apiKey;
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_AUTOREFERER, TRUE);
  curl_setopt($ch, CURLOPT_HEADER, 0);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_URL, $url);
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION, TRUE);
  $SummonerStatsJSON = curl_exec($ch);
  curl_close($ch);
  $stats = json_decode($SummonerStatsJSON);

  $url = 'https://euw.api.pvp.net/api/lol/euw/v2.5/league/by-summoner/' . $_SESSION['summ']->id . '?api_key=' . $apiKey;
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_AUTOREFERER, TRUE);
  curl_setopt($ch, CURLOPT_HEADER, 0);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  ...
  for ($i=0; $i < count($userTopChampions); $i++) {
    $currentTopChampion = $userTopChampions[$i]->championId;
    $_SESSION['bg_image'] = getChampionBackgroundPicture($currentTopChampion);
  }

  $leagues_array = $elo[$_SESSION['summ']->id];
  $first_league = $leagues_array[0];
  $tier = $first_league["tier"];
  $entries = $first_league["entries"];
  for ($i=0; $i < count($entries); $i++) {
    if($entries[$i]["playerOrTeamId"] == $_SESSION['summ']->id) {
 ...
 ?>
  ...
  <link rel="stylesheet" type="text/css" href="css/summonerPage_design.css">
...
<body style="background-image: url(<?php print $_SESSION['bg_image'] ?>)">

<div class="IndexLayoutWrap">

  <div id="preloader">
    <h1>LOADING... PLEASE WAIT!</h1>
    <h5>IF SUMMONER IS IN GAME IT TAKES LONGER WHILE THE WEBSITE IS IN DEVELOPMENT</h5>
  </div>

  <nav class="navbar">
    <a class="websiteLogo" href="index.php"><img src="img/Website_Logo.png" style="display: inline; margin-left: 25px; position: absolute; height: 65px; padding: 5px;"></img></a>
    <div class="container">
    ...
    <div class="topnav">
      <a class="active" onclick="switchNavigationDisplay('.userBASICInformation')" id="userinfotabbtn">USER INFO</a>
      <a onclick="switchNavigationDisplay('.userMatchHISTORY')" id="matchhistorytabbtn">MATCH HISTORY</a>
      <a onclick="switchNavigationDisplay('.userMANUAL')" id="usermanualtabbtn">USER MANUAL</a>
      <a onclick="switchNavigationDisplay('.userLiveGAMEInformation')" id="userliveinformationtabbtn">LIVE GAME</a>
    </div>
  </nav>
...
<div class="userINFOContainer">
  <br/><br/><br/><br/><br/><br/><br/><br/><br/>
    <div class="userINFO">
      <span class="userINFO_span">
          <img src="<?php print getDivisionPicture($tier, $division) ?>" class="user_rank_display" style="position: relative;"></img>
          <span class="summoner_name_display_userINFO"> <?php print $summonerName ?>
            <span id="summoner_name_display_userINFO_Border_Right"></span>
          </span>
      </span>
      <div class="refresh_button" onclick="refreshByButton()">REFRESH</div>
      <span class="lastRefreshedText">LAST REFRESHED : <span class="lastRefreshedNumText">15</span> SECONDS AGO</span>
    </div>
  </div>
  ...
  <div class="userSTATS_Start">
    ...
    <div class="userSTATS">
      ...
      <div class="userBASICInformation">
        <p class="userSUMMONER_LevelText">SUMMONER LEVEL <span class="userSUMMONER_Level"> <?php print $summonerLevel; ?> </span> </p>
...
/*
        ...
        for ($i=0; $i < count($userTopChampions); $i++) {
          $currentTopChampion = $userTopChampions[$i]->championId;
          $currentTopChampion = getChampionName($currentTopChampion);

          print '<p><b>TOP CHAMPION ' . $count . ' | ';
          print 'CHAMPION ID/NAME : ' . $currentTopChampion . ' | ';
          print 'CHAMPION LEVEL : ' . $userTopChampions[$i]->championLevel . '</p></b>';
          $count++;
        }
*/
//      print $summonerName . ' is ' . $tier . ' ' . $division;

        $statSummaries = $stats->playerStatSummaries;
        for ($i=0; $i < count($statSummaries); $i++) {
          // $statSummary->losses: sometimes losses isn't set
          $losses = property_exists($statSummaries[$i], 'losses')? $statSummaries[$i]->losses : '(not available)';
          print '<p><b>' . $statSummaries[$i]->playerStatSummaryType . '</b>: ' .
              $statSummaries[$i]->wins . ' wins, ' . $losses . ' losses</p>';
        }
      ...
      <div class="userMatchHISTORY">

        <div class = "matchHistoryRow" id="bluePlayerRow">
          <span class="historyMatchPlayer">MATCH PLAYED 0/0/0</span>
        </div>

      </div>

      <div class="userManual">
        <h1>USER MANUAL \ START</h1>
      </div>

<!-- SUMMONER LIVE GAME INFORMATION YOV STAR DET DA TE NE ČUJEM VEČ LMAO TRU LIT FAM DAB DAB -->
      <div class="userLiveGAMEInformation">
        ...
        if($LiveGameInfoJSON != null) {
          ...
          if($LiveGameInfo != null && $LiveGameID != null && $LiveGameDuration != null && $LiveGameMapID != null && $LiveGameParticipants != null) {
          ...
          print 'Failed to retrieve information for live game!';
        }
        ...
        <div class="infoRow"> <span> PLAYING <span style="font-weight: bold;">
         <?php print $LiveGameQueueConfigName; ?> <span style="font-weight: normal;"> ON <span style="font-weight: bold;">
        <?php print $MapName; ?> </span> </span> </span> </span>
        </div>
        <div class="infoRow">
          ...
          <div class="liveGamePlayersLeft">
            <div class = "LiveGamePlayerRow" id="bluePlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant1_ELOTier, $LiveGameParticipant1_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant1_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
            ...
            <div class = "LiveGamePlayerRow" id="bluePlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant2_ELOTier, $LiveGameParticipant2_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant2_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
            ...
            <div class = "LiveGamePlayerRow" id="bluePlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant3_ELOTier, $LiveGameParticipant3_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant3_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
            ...
            <div class = "LiveGamePlayerRow" id="bluePlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant4_ELOTier, $LiveGameParticipant4_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant4_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
            ...
            <div class = "LiveGamePlayerRow" id="bluePlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant5_ELOTier, $LiveGameParticipant5_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant5_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
          ...
          <div class="liveGamePlayersRight">
            ...
            <div class = "LiveGamePlayerRow" id="redPlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant6_ELOTier, $LiveGameParticipant6_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant6_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
            ...
            <div class = "LiveGamePlayerRow" id="redPlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant7_ELOTier, $LiveGameParticipant7_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant7_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
            ...
            <div class = "LiveGamePlayerRow" id="redPlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant8_ELOTier, $LiveGameParticipant8_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant8_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
            ...
            <div class = "LiveGamePlayerRow" id="redPlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant9_ELOTier, $LiveGameParticipant9_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant9_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
            ...
            <div class = "LiveGamePlayerRow" id="redPlayerRow">
              <div class="row">
                ...
                <div class="col s5 LiveGamePlayerChampion">
                  <img id="LiveGameDivisionPicture" src="<?php print getDivisionPicture($LiveGameParticipant10_ELOTier, $LiveGameParticipant10_ELODivision) ?>" data-position="left" data-delay="25" data-tooltip="<?php print $LiveGameParticipant10_ELODivision ?>" class="LiveGamePlayerChampionImage tooltipped"></img>
...
$(document).ready(function() {
  ...
  // Switch the navigation to the basic user information screen
  switchNavigationDisplay('.userBASICInformation');
  ...
  $(function () {
		$(window).scroll(function () {
			...
			if ($(this).scrollTop() > 25) {
				$('.topnav').slideUp('fast', 0);
			} else {
				$('.topnav').slideDown('fast', 0);
			}
		});
	});
  ...
  function animate() {
      for(i=0;i<3;i++) {
        $("#summoner_name_display_userINFO_Border_Right").fadeTo('fast', 0).fadeTo('slow', 1.0);
      }
      animate();
    }
  });
  ...
  if(currentLiveGameTimeDuration != null) {
  ...
  // Send the php variable of the live game id to javascript.
  var isInLiveGame = <?php echo(json_encode($LiveGameID)) ?>;

    // Check if live game id is null
    if(isInLiveGame == null) {
      // If it is hide the button
      $('#userliveinformationtabbtn').hide();
    } else {
      $('#userliveinformationtabbtn').show();
    }

  var searchedSummonerName = <?php echo(json_encode($summonerName)) ?>;
  if(searchedSummonerName != null) {
    var SummonerNames = document.getElementById("LiveGamePlayersBorder").querySelectorAll(".LiveGamePlayerSummonerName");
    var SummonerTiles = document.getElementById("LiveGamePlayersBorder").querySelectorAll(".LiveGamePlayerRow");
    var searchedSummonerTile;

    for (var i = 0; i < SummonerNames.length; i++) {
      if(SummonerNames[i].innerHTML.toLowerCase() == searchedSummonerName) {
        searchedSummonerTile = SummonerTiles[i];
        break;
      }
    }

    searchedSummonerTile.style.filter = 'grayscale(25%)';
  }
  ...
  // Animate the focused-like effect at the summoner name you're currently on

var currNav = ".userBASICInformation";
// This function allows you to switch between displays
function switchNavigationDisplay(navToSwitchTo) {

  $(".userBASICInformation").hide();
  $(".userLiveGAMEInformation").hide();
  $(".userMatchHISTORY").hide();
  $(".userManual").hide();

  $('#userinfotabbtn').removeClass("active");
  $('#matchhistorytabbtn').removeClass("active");
  $('#usermanualtabbtn').removeClass("active");
  $('#userliveinformationtabbtn').removeClass("active");

  if(currNav != navToSwitchTo) {
    $('html,body').animate({ scrollTop: $(".userINFO").offset().top}, 'slow');
  }

  currNav = navToSwitchTo;

  switch (navToSwitchTo) {
    case '.userBASICInformation':
      $('#userinfotabbtn').addClass("active");
      $(".userBASICInformation").fadeIn("fast");
      break;
    case '.userLiveGAMEInformation':
      $('#userliveinformationtabbtn').addClass("active");
      $(".userLiveGAMEInformation").fadeIn("fast");
      break;
    case '.userMatchHISTORY':
      $('#matchhistorytabbtn').addClass("active");
      $(".userMatchHISTORY").fadeIn("fast");
      break;
    case '.userMANUAL':
      $('#usermanualtabbtn').addClass("active");
      $(".userManual").fadeIn("fast");
      break;
    default:
  }
...
// Refresh the current summoner
function refreshByButton() {
  ...
  var w = $(".refresh_button");
  $.ajax({
    type: 'POST',
    url: 'php/refreshSummoner.php',
    data: {
      summNameInput: "refresh",
    },
...
function getDivisionPicture($tier, $division) {
  //if($summonerLevel < 29) { return 'http://i.imgur.com/7eqikR2.png'; }
  if ($tier == "BRONZE") {
    if ($division == "I") {
...
function getELO($ID, &$selfTier, &$selfDivision) {
  ...
  if($first_league["queue"] == "RANKED_SOLO_5x5"){
    $tier = $first_league["tier"];
    //print $tier;
    $selfTier = $tier;
    $entries = $first_league["entries"];
    ...
    $selfDivision = 'PROVISIONAL';
    $selfTier = 'PROVISIONAL';
